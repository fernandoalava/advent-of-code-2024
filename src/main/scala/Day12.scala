...
""".split("\n").toVector
  ...
  def calculatePriceWithBulkDiscount(
    ...
    val corners = region.foldLeft(0)((acc, value) => {
      ...
      if (up !=  Some(value._1)  && right != Some(value._1) && upRight != Some(value._1)) {
        c += 1
      }
      ...
      if (up != Some(value._1) && left != Some(value._1) && upLeft != Some(value._1)) {
        c += 1
      }
      ...
      if (down != Some(value._1) && left != Some(value._1) && downLeft != Some(value._1)) {
        c += 1
      }
      ...
      if (down != Some(value._1) && right != Some(value._1) && downRight!= Some(value._1)) {
        c += 1
      }
    ...
    println(s"${region.head._1} - Corners: ${corners}")
    return area * corners
  }
...
@main def day12Main() = {
  ...
  // println(puzzle1(inputTest2))
  // println(puzzle1(getInput()))
  println(puzzle2(inputTest3))
